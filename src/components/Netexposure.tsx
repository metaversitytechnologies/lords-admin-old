    ...
    <div>
      <div id="net-exposure" className="tab-pane net-exposure">
        ...
        <div className="float-right">
          ...
          <button className="btn btn-secondary m-l-10" onClick={fetchData}>Refresh</button>
        </div>
        <div>
          <table className="table">
            <thead>
              <tr>
                <th style={{ minWidth: "10%", maxWidth: "10%", width: "10%" }}></th>
                <th style={{ minWidth: "8%", maxWidth: "8%", width: "8%" }}></th>
                <th className="text-center" style={{ minWidth: "8%", maxWidth: "8%", width: "8%" }}>
                  Stake
                </th>
                <th className="text-center" style={{ minWidth: "8%", maxWidth: "8%", width: "8%" }}>
                  1
                ...
                <th style={{ minWidth: "18%", maxWidth: "18%", width: "18%" }}></th>
                <th style={{ minWidth: "16%", maxWidth: "16%", width: "16%" }}></th>
                <th className="text-center" style={{ minWidth: "8%", maxWidth: "8%", width: "8%" }}>
                  Stake
                </th>
                <th className="text-center" style={{ minWidth: "8%", maxWidth: "8%", width: "8%" }}>
          ...
          {data.map((item, index) => (
            <div key={index}>
              <table className="table m-b-5">
                ...
                <tbody>
                  {Object.values(item.marketDetail).map((market: any, subIndex) => (
                    <tr key={subIndex}>
                      ...
                      <td className="icon-group-cell">
                        <ul className="icon-group">
                          <li className="cell-market-workstation">
                            <Link
                              to={`/gamedetailnew/4/${item.matchId}`}
                      ...
                      <td className="cell-stake">{market.liability?.toFixed(2) || '0.00'}</td>
                      <td className="cell-selection-1">
                        <span className={market.pnl1 >= 0 ? "positive" : "negative"}>
                          {market.pnl1?.toFixed(2) || '0.00'}
                        </span>
                      </td>
                      <td className="cell-selection-2">
                        <span className={market.pnl2 >= 0 ? "positive" : "negative"}>
                          {market.pnl2?.toFixed(2) || '0.00'}
                        </span>
                      </td>
                      <td className="cell-selection-3">
                        <span className={market.pnl3 >= 0 ? "positive" : "negative"}>
                ...
                {item.fancyDetail && Object.values(item.fancyDetail).length > 0 && (
                  <tbody>
                    ...
                    {Object.values(item.fancyDetail).map((fancy: any, fancyIndex) => (
                      <tr key={fancyIndex}>
